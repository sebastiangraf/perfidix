...
public class PerfidixContainerInitializer extends ClasspathContainerInitializer {
	public static final String BENCH_CONTAINER_ID="org.perfidix.Perclipse.PERFIDIX_CONTAINER";
	public static final IStatus NOT_SUPPORTED= new Status(IStatus.ERROR, PerclipseActivator.PLUGIN_ID, ClasspathContainerInitializer.ATTRIBUTE_NOT_SUPPORTED, new String(), null);
	public static final IStatus READ_ONLY = new Status(IStatus.ERROR, PerclipseActivator.PLUGIN_ID, ClasspathContainerInitializer.ATTRIBUTE_READ_ONLY, new String(), null);
	...
	@Override
	public void initialize(IPath cpath, IJavaProject project) throws CoreException {
		if(isValidPerfidixContainerPath(cpath)){
			PerfidixContainer perfidixContainer= getNewContainer(cpath);
			JavaCore.setClasspathContainer(cpath, new IJavaProject[]{project}, new IClasspathContainer[]{perfidixContainer}	, null);
		}
		...
		if(entries.length==1 && isValidPerfidixContainerPath(containerPath)){
			...
			for (int i = 0; i < extraAttributes.length; i++) {
				...
				if(attrib.getName().equals(IClasspathAttribute.JAVADOC_LOCATION_ATTRIBUTE_NAME)){
					IPreferenceStore preferenceStore= PerclipseActivator.getDefault().getPreferenceStore();
					preferenceStore.setValue("This is the Perfidix JavaDoc", attrib.getValue());
				}
			}
			try {
	...
	private void rebindClasspathEntries(IJavaModel javaModel,
		...
		if(!affectedProject.isEmpty()){
			IJavaProject[] affected=(IJavaProject[]) affectedProject.toArray(new IJavaProject[affectedProject.size()]);
