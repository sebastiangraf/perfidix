...
public class PerfidixLaunchShortcut implements ILaunchShortcut {
	...
	public void launch(ISelection selection, String mode) {
		...
		if (selection instanceof IStructuredSelection) {
			...
			//an dieser Stelle kann man die View mal testen
	...
	private void launchType(IJavaElement search, String mode) {
		...
		try {
			...
			for (int i = 0; i < types.length; i++) {
				System.out.println("asdf: "+types[i].getElementName());
			}
	 ...
	 * @throws LaunchCancelledByUserException
	 */
	public ILaunchConfiguration findOrCreateLaunchConfiguration(String mode,
			PerfidixLaunchShortcut registry,
			PerfidixLaunchDescription description)
			throws LaunchCancelledByUserException {

		ILaunchConfiguration config = registry.findLaunchConfiguration(mode,
				description);
	 ...
	 * @throws LaunchCancelledByUserException
	 */
	public ILaunchConfiguration findLaunchConfiguration(String mode,
			PerfidixLaunchDescription description)
			throws LaunchCancelledByUserException {

		ILaunchConfigurationType configType = getPerfidixLaunchConfigType();
		List<ILaunchConfiguration> candidateConfigs = new Vector<ILaunchConfiguration>(
				0);
	...
	protected ILaunchConfiguration createConfiguration(IJavaProject project,
			String name, String mainType, String container, String benchName) {
		...
		try {
			...
			wc.setAttribute(PerfidixLaunchConfiguration.LAUNCH_CONTAINER_ATTR,
					container);
			if (benchName.length() > 0)
				wc.setAttribute(PerfidixLaunchConfiguration.BENCH_NAME_ATTR,
						benchName);
			config = wc.doSave();
	...
	protected String getContainerLabel(IJavaElement container) {
		String name = JavaElementLabels.getTextLabel(container,
				JavaElementLabels.ALL_FULLY_QUALIFIED);
		return name.substring(name.lastIndexOf(IPath.SEPARATOR) + 1);
	}
	
	//TestMethode für Tree
	private  TreeDataProvider[] fillElements(String elementName){
		return new TreeDataProvider[] {new TreeDataProvider(elementName, 123, 321)};
