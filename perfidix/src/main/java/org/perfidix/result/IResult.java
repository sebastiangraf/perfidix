...
public interface IResult {
     ...
     * an instance of SingleResult is being created and fed with the
     * 
     * @author axo
     * @since 2005
     */
    public class SingleResult extends Result {
        ...
        /**
         * i think there must be a simpler approach to this problem. the input
         * array given could contain some Benchmark.LONG_NULLVALUEs, which tell
         * us that at that iteration, the method or whatever wasn't called. for
         * the current computations it's not important to show these values, and
         * such i remove these and return a copy of the array. unfortunately,
         * this renders the algorithm's complexity to O(n^2) when any null value
         * is found, and to O(n) when no null value was found in the first run.
         * because i first have to iterate through the original array to
         * estimate the result array's length, and then i have to copy the
         * values into the newly allocated array. if there's a shorter method
