...
public abstract class Result implements IResult {
  ...
  /**
   * if any result computation overflows (as can happen with
   * the nanometer on slow methods), this value will be taken as a response.
   * i know that the correct way would be to throw an exception, 
   * but this means that i would have to perform try/catch routines 
   * on almost every bloody computation, and i am really too lazy to do
   * this.
   * 
   */
  ...
  /**
   * temp cache.
   * this 'caching' ist not implemented as it should be.
   * the reason is that normally i should have implemented it 
   * with an ArrayList containing Number objects or by simply using
   * a proxy Result which computes and stores the values.
   * after a simple benchmark i saw that using objects like that
   * would result in a performance loss of about 40-60 percent with
   * the computations.
   * so i changed this to use the simplistic but redundant approach
  ...
  /**
   * initializes itself.
   * needed for the xstream loading.
   *
   */
  ...
  /**
   * needed for the xstream implementation.
   * obsolete
  ...
  /**
   * simple setter.
   * @param someName to give the result a name.
   */
  ...
  /** 
   * computes the number of data fields available.
   * this is the number of data items for non-skipped runs.
  ...
  /** 
   * returns the artithmetic mean of the result set.
   * avg() is an alias for this method. 
   * @see #avg
  ...
  /**
   * computes the middle value of the set distribution.
   * the value of this median needs not necessarily be within
   * the given resultSet length, but will be the computed absolute
   * value.
   * the result set values are sorted first, and 
   * if the result set is even, 0.5 * ( the two middle values) 
   * is being taken.
   * if the result set is odd, the middle value is being taken.
   * 
  ...
  /**
   * computes the average value of the data set.
   * alias for mean().
   * @see #mean()
  ...
  /**
   * returns the confidence 99 interval since it's a +/- thing, the 
   * first one is
   * the "+" one, the second array value is the "-" one.
   ...
   * TODO enum implementation of CONF ?
   */
   ...
   * @return string
   */
  protected final String toString(final int indent, final boolean displayRawData) {
   ...
   * @param r the result to compare with
   */
   ...
   * only to override the original implementation, since it's not 
   * used here.
   * @return always 0 
   */
