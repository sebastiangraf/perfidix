...
public interface IRandomizer {
   ...
   * @param m the method to ask for.
   * @return boolean whether it should be run or not.
   */
  public boolean shouldRun(Method m);

  /**
   * the default randomizer returns always true, which is the default behavior.
   */
  ...
  /**
   * the Randomizer allows using probabilities computing 
   * whether to run a certain
   * method or not.
   */
  public class Randomizer extends IRandomizer.DefaultRandomizer {
    ...
    /** 
     * @param hashes the hashtable to look up.
     * 
     */
     ...
     * @param m the method 
     * @param probability the probability to use this method.
     * @throws ProbabilityInvalidException on invalid data.
    ...
    public boolean shouldRun(final Method m) {
      ...
      //System.out.println("should " + (result ? "" : " not ") + " run " + m.getName());
      return result;
    }
  ...
  /** 
   * this is a rather simple randomizer.
   * it returns true every second call, starting with 
  ...
  public class Switcher implements IRandomizer {
    ...
    /**
     * tells whether to run the method or not.
     * for each shouldRun() call, it will return a
     * row of true, false, true, false, true, false.
