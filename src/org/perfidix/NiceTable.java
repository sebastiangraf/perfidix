 ...
 * tables.
 * 
 * TODO a table could recursively contain other tables, this would affect also
 *       the height of table rows. make this one truly 2-dimensional. a word on
 *       the orientations: at the current point of development, the orientation
 *       works on COLUMNS and NOT on ROWs. so you can set the orientation of a
 *       full ROW, but you cannot set the orientation for each field[x][y] in
 *       the table.
 * CHANGELOG:
 * --------------------------------------------------------------------
 * 02-02-2006: tested the newline problems and made them work.
 *             it is now possible to create tables like
 *             | a | b |
 *             | c |   | 
 *             with the definition of String s = {"a\nc","b"} ;
 *             
 * 08-02-2006: refactored the whole class in to get rid of the 
 *             formatter warnings. removed the numColumns variable. 
 * @author axo
 * @since 2005-10-21
 */
public class NiceTable {
   ...
   * implemented some more things about it, and since i'm not thinking of that,
   * i make this one final.
   */
   ...
   * 
   * @example: column a contains the string "a" in the first row. so the column
   *           length for the first row is '1'. this gets updated whenever
   *           there's a new row to insert.
   */
  ...
  /**
   * sets the orientation of the column with index i. available orientations are
   * NiceTable.LEFT, NiceTable.RIGHT, NiceTable.CENTER take care to use the
   * correct column index.
   * @param columnIndex integer 
   * @param orientation the orientation of the row content.
   */
   ...
   * @param columnIndex the index of the column for which 
   *                    to fetch the width.
  ...
  /** 
   * adds a string row. checks that the strings added do not
   * contain newlines, because if so, it has to split them 
   * in order to make them fit the row. 
  ...
  private interface Drawable {
    ...
    /**
     * returns the minimum width required to display the contents. this has to
     * be fetched from all rows at display time.
     */
  ...
  private final class Header implements Drawable {
    ...
    private Header(
        ...
        final char theEnclosing,
        final int theOrientation,
        final NiceTable theTable) {

      this.orientation = theOrientation;
      this.enclosing = theEnclosing;
      this.table = theTable;
      this.title = enclosing + SPACE + contentToDisplay + SPACE + enclosing;
    }
    ...
    public String draw() {
      ...
      return theBORDER
          + Util.pad(title, enclosing, table.getTotalWidth() - 2, orientation)
          + theBORDER
          + NEWLINE;
    }
  ...
  public final class Row implements Drawable {
     ...
     * @param myParent the parent pointer.
     */
    ...
    /**
     * sets the column data to new values. take care that the inserted array has
     * to have the same size as the available columns.
     * @param d the data
     *
     */
    ...
    /**
     * this is soooo redundant and i don't like it. but i need it. merde. tell
     * me if there's a better way to do this.
     * @param d the data
     */
  ...
  public static final class Util {
     ...
     * method, and i accept new proposals. an example will show what this one
     * does.
     * 
     * 
     * <pre>
    ...
    public static String pad(
        final String data,
        final char doPadWithThis,
        final int totalWidth,
        final int orientation) {

      String pad =
          repeat("" + doPadWithThis, Math.max(0, totalWidth - data.length()));
    ...
    /** 
     * 
     * @param glue the string to implode with
     * @param what the object array to display.
     * @return string.
     */
    ...
    /**
     * there has to be a utility for this, but i could not find it, i am very
     * sorry to say. if anyone finds the utility, tell me and it'll be factored
     * out in no time.
     * @param glue bla
     * @param what bla
     * @return the result
     ...
     * @param s the string to look upon.
     ...
     * it's important to note that only newlines within the string
     * are important, not the newlines at the front or the end of
     * the string.
     */
    ...
    /**
     * Creates a matrix according to the number of new lines 
     * given into the method.
     * @return the matrix. 
  ...
  /** 
   * sets the column delimiter, in order to allow different delimiters
   * for different contexts. 
   * @param c the border string.
   */
