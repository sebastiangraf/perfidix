...
/**
 * the result container contains more results. it is by definition recursive, 
 * so it can handle as much diversity as possible
 * 
...
public abstract class ResultContainer<ResultType extends Result> extends Result {
  ...
  /**
   * returns the full result set on all children. this works recursively.
   * the recursion takes place by calling each child's sum() operation.
   * 
  ...
  /**
   * 
   * @param meterName the unique identifier for the meter involved.
  ...
  /**
   * 
   * @param m the meter to fetch the result set for.
  ...
  private ArrayList<SingleResult> getSingleResultsFor(final IMeter meter) {
    ...
    for (int i = 0; i < children.size(); i++) {
      ...
      if (tmp instanceof ResultContainer) {
        theList.addAll(((ResultContainer) tmp).getSingleResultsFor(meter));
      }
    }
    return theList;
  }
   ...
   * @param m the meter which is used.
  ...
  private void pushUnique(
      final ArrayList<IMeter> into,
      final ResultContainer from) {
