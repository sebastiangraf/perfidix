...
public class APITest extends PerfidixTest {
	...
	public void testMultipleAvg() {
		...
		IResult.SingleResult r1 = Perfidix.createSingleResult("testMultipleAvg()A", set[0]);
		IResult.SingleResult r2 = Perfidix.createSingleResult("testMultipleAvg()B", set[1]);
		ResultContainer<IResult.SingleResult> cnt = new IResult.MethodResult("test_multipleAvg");
		cnt.append(r1);
		cnt.append(r2);
	...
	public void testMultipleMin() {
		...
		ResultContainer<IResult.SingleResult> cnt = new IResult.MethodResult("has a name");
		cnt.append(r1);
		cnt.append(r2);
		cnt.append(r3);
	...
	public void testRunMethods() {
		...
		bench.add(cl);
		bench.run();
		assertTrue("should have invoked the first method",
				cl.firstMethodInvoked);
		assertTrue("should have invoked the second method",
				cl.secondMethodInvoked);
	...
	public void testInvocations() {
		...
		int invokeCount = 5;
		bm.run(invokeCount);
		assertEquals("it should run as many invocations i tell it....",
				invokeCount, ding.numInvocations);
	...
	public void testFibIsWorking() {
		...
		int[][] fResults = { { 1, 1 }, { 2, 2, }, { 3, 3 }, { 4, 5 }, { 5, 8 },
				{ 6, 13 } };
	...
	public void testAvgContainer() {
		...
		ResultContainer<IResult.SingleResult> res = new IResult.MethodResult("test");
		for (int i = 0; i < testResultSet.length; i++) {
			res.append(new IResult.SingleResult("bla" + i, testResultSet[i],
					Perfidix.defaultMeter()));
		}
		assertEquals(3l, res.resultCount());
	...
	public void testVariance() {
		...
		assertEquals("expecting 479.333, but receiving " + resVariance,
		...
		assertEquals("expected " + res2Expected + ", but received "
				+ res2Variance, res2Variance, res2Expected,
	...
	public void testSquareSum() {
		...
		assertEquals(expectedDeviation, ra.getStandardDeviation(),
				PerfidixTest.EPSILON);
	...
	public void testConfidence() {
		...
		double aShouldBe = 2.576 * (res.getStandardDeviation() / Math.sqrt(res
				.resultCount()));
	...
	public void testStddev() {
		...
		ResultContainer<IResult.SingleResult> c = new IResult.MethodResult("bla");
		c.append(a);
		c.append(b);

		assertEquals(expStandardDeviation, a.getStandardDeviation(),
				PerfidixTest.EPSILON);
		assertEquals(expStandardDeviation, b.getStandardDeviation(),
				PerfidixTest.EPSILON);
